...
class Dataset_SchNet_DimeNet(torch_geometric.data.Dataset):
    def __init__(self, property_type, mols, targets = None, ligand_ID = None, active_conformer = None, atom_ID = None, bond_ID_1 = None, bond_ID_2 = None, remove_Hs_except_functional = False):
        super(Dataset_SchNet_DimeNet, self).__init__()
        ...
        self.active_conformer = active_conformer
    ...
    def __getitem__(self, key):
        ...
        active_conf = None 
        if self.active_conformer is not None:
            active_conf = torch.as_tensor(self.active_conformer[key], dtype = torch.long)
        ...
        data.active_conformer = active_conf
        
        if self.remove_Hs_except_functional:
            acid_substructure = Chem.MolFromSmarts('[CX3](=O)[OX2H1]')
            acid_indexsall = mol.GetSubstructMatches(acid_substructure)
            amine_substructure = Chem.MolFromSmarts('[NH2]C')
            amine_indexsall = mol.GetSubstructMatches(amine_substructure)
            sec_amine_substructure = Chem.MolFromSmarts('[H]N([C])[C]')
