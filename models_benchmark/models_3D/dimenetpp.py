...
class DimeNetPlusPlus(torch.nn.Module):
    ...
    def __init__(
        ...
        #out_channels: int = 1, # not used due to adapted readout
        num_blocks: int = 4,
        int_emb_size: int = 128,
        ...
        self.output_blocks = torch.nn.ModuleList([
            OutputPPBlock(num_radial, 
                          hidden_channels, 
                          out_emb_channels, 
                          out_emb_channels, # was out_channels (==1) in DimeNetPlusPlus original code; changed here for new readout
                          num_output_layers, 
                          act) for _ in range(num_blocks + 1)
        ])
    ...
    def forward(self, z, pos, batch, atom_features = None):
        ...
        Args:
            ...
            pos (Tensor): Coordinates of each atom with shape
                :obj:`[num_atoms, 3]`.
            batch (LongTensor): Batch indices assigning each atom to
        ...
        # ablate 3D information
        if self.ablate_3D:
            rbf = torch.ones_like(rbf)
            sbf = torch.ones_like(sbf)
        ...
        h = self.output_blocks[0](x, rbf, i, num_nodes=pos.size(0)) # node embeddings

        # Interaction blocks.
        for interaction_block, output_block in zip(self.interaction_blocks, self.output_blocks[1:]):
            x = interaction_block(x, rbf, sbf, idx_kj, idx_ji)
            h = h + output_block(x, rbf, i, num_nodes=pos.size(0)) # node embeddings
